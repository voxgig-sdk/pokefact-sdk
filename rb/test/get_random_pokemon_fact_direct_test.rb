...
class GetRandomPokemonFactDirectTest < Minitest::Test
  def test_direct_list_get_random_pokemon_fact
    setup = get_random_pokemon_fact_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-get_random_pokemon_fact", setup[:live] ? "live" : "unit")
    if _should_skip
...
def get_random_pokemon_fact_direct_setup(mockres)
  ...
  if live
    ...
    client = PokefactSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
