...
class GetRandomPokemonFactEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = PokefactSDK.test(seed, nil)
    seen = base.GetRandomPokemonFact(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GetRandomPokemonFact(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
