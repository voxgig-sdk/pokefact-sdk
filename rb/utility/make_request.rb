...
module PokefactUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = PokefactResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PokefactResponse.new(fetched)
    else
