...
class GetRandomPokemonFactEntity
  ...
  def make
    opts = @_entopts.dup
    GetRandomPokemonFactEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PokefactHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PokefactHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GetRandomPokemonFactListMatch, Hash, nil] match filter (any subset of
  #   GetRandomPokemonFact fields); defaults to nil, treated as an empty match that lists all.
