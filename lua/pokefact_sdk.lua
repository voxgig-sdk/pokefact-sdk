...
local PokefactSDK = {}
PokefactSDK.__index = PokefactSDK
...
PokefactSDK._make_feature = _make_feature


function PokefactSDK.new(options)
  local self = setmetatable({}, PokefactSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PokefactSDK:options_map()
...
function PokefactSDK:get_utility()
  return Utility.copy(self._utility)
end


function PokefactSDK:get_root_ctx()
  return self._rootctx
end


function PokefactSDK:prepare(fetchargs)
...
function PokefactSDK:direct(fetchargs)
...
-- by passing it.
function PokefactSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PokefactSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PokefactSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:GetRandomPokemonFact():list() / client:GetRandomPokemonFact():load({ id = ... })
...
function PokefactSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PokefactSDK.new(sdkopts)
  sdk.mode = "test"
