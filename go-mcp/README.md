...
[MCP](https://modelcontextprotocol.io) server exposing the Pokefact SDK as
two agent tools — `pokefact_list` and `pokefact_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokefact-mcp)
make build

# 2. Provide credentials via the environment
export POKEFACT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pokefact \
  -- /absolute/path/to/pokefact-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pokefact-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pokefact_list: first page of records
{ "entity": "get_random_pokemon_fact" }
{ "entity": "get_random_pokemon_fact", "query": { } }

// pokefact_load: one record by id
{ "entity": "get_random_pokemon_fact", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokefact-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pokefact \
     -- "$PWD"/dist/*/pokefact-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pokefact_list` and `pokefact_load` tools now appear
   in new sessions. Ask the agent to *"list get_random_pokemon_fact using pokefact"*
   and it calls `pokefact_list` with `{"entity":"get_random_pokemon_fact"}`.
...
```sh
export POKEFACT_APIKEY=sk_live_xxx            # API key
export POKEFACT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pokefact_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pokefact_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POKEFACT_APIKEY` | API key sent with every request. |
| `POKEFACT_BASE` | Optional override of the API base URL. |
...
```sh
./pokefact-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pokefact_load","arguments":{"entity":"get_random_pokemon_fact","query":{"id":1}}}}'
```
