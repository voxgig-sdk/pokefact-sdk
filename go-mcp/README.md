...
MCP server exposing the Pokefact SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `pokefact_list` | `entity`, optional `query` map | First page of records as JSON |
| `pokefact_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `pokefact_list` and
`pokefact_load` tools become available in new sessions.
