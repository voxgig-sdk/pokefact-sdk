...
//     (\`new PokefactSDK(...)\` / \`PokefactSDK.test(...)\` become
//     \`PokefactSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_random_pokemon_fact":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PokefactSDK(...)\` and any
// \`PokefactSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PokefactSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PokefactSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
