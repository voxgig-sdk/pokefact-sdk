...
class TestGetRandomPokemonFactEntity:
    ...
    def test_should_stream(self):
        ...
        base = PokefactSDK.test(seed, None)
        seen = list(base.GetRandomPokemonFact(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = PokefactSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.GetRandomPokemonFact(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        get_random_pokemon_fact_ref01_list_result = get_random_pokemon_fact_ref01_ent.list(get_random_pokemon_fact_ref01_match, None)
