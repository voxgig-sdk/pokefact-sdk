# Pokefact SDK test runner

from __future__ import annotations
import os
import json

from pokefact_sdk.utility.voxgig_struct import voxgig_struct as vs


class PokefactTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PokefactTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PokefactTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PokefactTestRunner.getenv("POKEFACT_TEST_LIVE")
        override = PokefactTestRunner.getenv("POKEFACT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PokefactTestRunner.getenv(key)
        ...
        explain = PokefactTestRunner.getenv("POKEFACT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["POKEFACT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PokefactTestRunner._test_control is not None:
            return PokefactTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PokefactTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PokefactTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PokefactTestRunner.load_test_control()
...
def load_env_local():
    PokefactTestRunner.load_env_local()


def env_override(m):
    return PokefactTestRunner.env_override(m)


def entity_data(v):
    return PokefactTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PokefactTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PokefactTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PokefactTestRunner.load_test_control()


def live_client_options():
    return PokefactTestRunner.live_client_options()


def live_delay_ms():
    return PokefactTestRunner.live_delay_ms()
