...
class FtCtrl extends PokefactControl
{
...
class FtClient
{
    ...
    private PokefactUtility $utility;

    public function __construct(PokefactUtility $utility, array $options)
    {
    ...
    public function get_utility(): PokefactUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PokefactContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PokefactContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PokefactContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PokefactConfig::make_feature($name);
    ...
    private static function buildUrl(PokefactSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PokefactError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PokefactResponse($fetched);
        }
    ...
    private function fail(PokefactContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PokefactContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
