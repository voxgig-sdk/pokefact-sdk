...
class GetRandomPokemonFactEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = PokefactSDK::test($seed, null);
        $seen = iterator_to_array($base->GetRandomPokemonFact(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = PokefactConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = PokefactSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->GetRandomPokemonFact(null)->stream("list", null, null) as $item) {
