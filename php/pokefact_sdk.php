...
class PokefactSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PokefactHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PokefactHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PokefactFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PokefactFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PokefactUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PokefactHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PokefactHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PokefactHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PokefactError($op . "_allow",
                "PokefactSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PokefactHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PokefactError("graphql_error",
                "PokefactSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetRandomPokemonFact($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_random_pokemon_fact === null) {
                $this->_get_random_pokemon_fact = new GetRandomPokemonFactEntity($this, null);
            }
            return $this->_get_random_pokemon_fact;
        }
        return new GetRandomPokemonFactEntity($this, $data);
    }
