...
class PokefactNetsimFeature extends PokefactBaseFeature
{
    ...
    private function _simulate(PokefactContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(PokefactContext $ctx, array $applied): void
    {
        $client = $this->client;
