...
// Pokefact SDK GetRandomPokemonFact entity
...
class GetRandomPokemonFactEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_random_pokemon_fact";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetRandomPokemonFactEntity($this->_client, $opts);
    }

    /**
     * @param GetRandomPokemonFact|array $args GetRandomPokemonFact data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PokefactHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetRandomPokemonFact|array The current GetRandomPokemonFact data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetRandomPokemonFact fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PokefactHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetRandomPokemonFact fields).
     */
     ...
     *
     * @param GetRandomPokemonFactListMatch|array|null $reqmatch Match filter (any subset
     *   of GetRandomPokemonFact fields) as an assoc-array; GetRandomPokemonFactListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetRandomPokemonFact[]|array A list of GetRandomPokemonFact items as assoc-arrays at
     *   the SDK boundary; throws PokefactError on failure (item-5 convention).
     */
