...
//
// NewPokefactSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.PokefactSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewPokefactSDK(opts)
}
...
func fcCandidates(client *sdk.PokefactSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.PokefactSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.PokefactSDK, name string) any {
