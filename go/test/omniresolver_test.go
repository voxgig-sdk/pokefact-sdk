...
import (
	...
	omni "github.com/voxgig-sdk/pokefact-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PokefactError); is {
				out := map[string]any{
					"name":    "PokefactError",
...
func omniCtx(arg any, client *sdk.PokefactSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PokefactSDK); is {
			client = live
