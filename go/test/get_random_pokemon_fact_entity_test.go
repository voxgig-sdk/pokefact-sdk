...
func TestGetRandomPokemonFactEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetRandomPokemonFact(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetRandomPokemonFact(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getRandomPokemonFactRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_random_pokemon_fact")))
