// Command Pokefact-cli is a CLI + REPL for the Pokefact SDK,
...
const prompt = "pokefact"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_random_pokemon_fact"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      POKEFACT_APIKEY (api key), POKEFACT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: POKEFACT_APIKEY carries the API key and
	// POKEFACT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("POKEFACT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("POKEFACT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPokefactSDK(opts)
