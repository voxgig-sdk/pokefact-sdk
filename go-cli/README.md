...
boru-driven command-line client **and** interactive REPL for the Pokefact
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokefact-cli)
make build
...
./pokefact-cli list get_random_pokemon_fact

# 5. Override the API base URL for a single call
POKEFACT_BASE=https://api.example.com ./pokefact-cli list get_random_pokemon_fact

# 6. No arguments -> interactive REPL
./pokefact-cli
pokefact> list get_random_pokemon_fact
pokefact> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokefact-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pokefact-cli list get_random_pokemon_fact
   ```
...
Run with no arguments to open a REPL (prompt `pokefact>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_pokemon_fact`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pokefact-cli`. |
...
(`./pokefact-cli <expr>`) and interactively (the REPL), and expressions compose the
